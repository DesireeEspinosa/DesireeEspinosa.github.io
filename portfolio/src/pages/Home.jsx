...
export default function Home() {
    ...
    return (
        <section className="relative min-h-[calc(100vh-72px-50px)] bg-bg overflow-hidden flex flex-col justify-end px-20 pb-0">
            ...
            <h1 className="font-display font-semibold text-[clamp(48px,6vw,68px)] text-cream leading-[1.05] mb-2">
                Frontend developer
            </h1>
            ...
            <div className="relative -mx-20 border-t border-b border-neon/10 overflow-hidden mb-5">
                <div className="flex gap-5 px-3 py-1 border-b border-neon/5">
                    {Array.from({ length: 60 }).map((_, i) => (
                        <div
                            key={i}
                            className="w-4 h-2 rounded-xs bg-bg border border-neon/10 shrink-0"
                        />
                    ))}
                </div>

                {/* Ticker */}
                <div className="flex overflow-hidden py-1.5 bg-bg2/40">
                ...
                <div className="flex gap-4.5 px-3 py-1 border-t border-neon/5">
                    {Array.from({ length: 60 }).map((_, i) => (
                        <div
                            key={i}
                            className="w-4 h-2 rounded-xs bg-bg border border-neon/10 shrink-0"
