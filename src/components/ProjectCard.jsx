export default function ProjectCard({ project }) {
    const { name, year, type, desc, stack, accent, images, client } = project;

    return (
        <div
            className="relative w-75 h-115 shrink-0 flex flex-col bg-bg2 border border-white/5 cursor-pointer transition-transform hover:scale-[1.02]"
                ...
                <div className="flex items-center justify-between border-t border-white/4 pt-3 mt-auto shrink-0">
                    {client.url ? (
                        <a
                            ...
                            className="flex items-center gap-1.5 transition-opacity opacity-50 hover:opacity-90"
                            style={{ color: `var(${accent})` }}
                        >
                            <span
                            ...
                            <span className="font-mono text-xs tracking-wide">
                                {client.label === false ? client.name : `Client: ${client.name}`} ↗
                            </span>
                        </a>
                    ) : (
                        <span
